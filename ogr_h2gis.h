...
inline graal_isolatethread_t* GetOrAttachThread() { return h2gis_wrapper_get_thread(); }
...
struct H2GISColumnInfo {
    ...
    std::string dataType;       // DATA_TYPE (e.g., "INTEGER", "GEOMETRY", "CHARACTER VARYING")
    int ordinalPosition;        // ORDINAL_POSITION
    std::string geometryType;   // GEOMETRY_TYPE (e.g., "MULTIPOLYGON Z", "POINT Z") - empty for non-geometry
...
inline OGRFieldType MapH2DataType(const std::string& h2Type) {
    ...
    if (h2Type == "BIGINT") 
        return OFTInteger64;
    if (h2Type == "REAL" || h2Type == "DOUBLE PRECISION" || h2Type == "FLOAT" || h2Type == "DECIMAL" || h2Type == "NUMERIC")
        return OFTReal;
    if (h2Type == "BOOLEAN")
...
public:
    ...
    OGRH2GISLayer(OGRH2GISDataSource *poDS, 
...
public:
    ...
    long long           GetConnection() { return m_hConnection; }
    void*               GetThread() { return m_hThread; }
};
